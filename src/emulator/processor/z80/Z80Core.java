...
/*
 * Original code located here: http://code.google.com/p/z80-cpu/
 * 
 * Modified by me :)
 * - changed the package name
 * - added executeCycles() method 
 * - modified (fixed?) interrupt handling (according to mame z80 code)
 */

package emulator.processor.z80;

import emulator.IBaseDevice;
import emulator.IIRQHandler;
import emulator.IMemory;
import emulator.ProcessorException;
...
public class Z80Core
{
	...
	public static final int HOLD_LINE = 2;
	
	
	public static final int INPUT_LINE_IRQ0 = 0;
	public static final int NCS800_RSTA = 1;
	public static final int NCS800_RSTB = 2;
	public static final int NCS800_RSTC = 3;
	public static final int Z80_INPUT_LINE_WAIT = 4;
	public static final int Z80_INPUT_LINE_BUSRQ = 5;

	public static final int INPUT_LINE_NMI = 32;
	public static final int INPUT_LINE_RESET = 33;
	public static final int INPUT_LINE_HALT = 34;
	...
	private IIRQHandler				irqHandler;
	...
	private int irqState = CLEAR_LINE;
	private int nmiState = CLEAR_LINE;
	private boolean afterEI = false;
	 ...
	 */
	public Z80Core(IMemory ram, IBaseDevice io, IIRQHandler irqHandler)
	{
		this.ram = ram;
		this.io = io;
	...
	private void takeInterrupt() {
		...
		int irqVector = irqHandler.handleIrq(this, 0, interruptMode);
		
		// Interrupt mode 2. Call [i:databyte]
		if (interruptMode == 2) {
			irqVector = (irqVector & 0xff) | (reg_I << 8);
			//push PC
			dec2SP();
			ram.writeWord(reg_SP, reg_PC);
			
			reg_PC = ram.readWord(irqVector);
			tStates += 17 + 2; // 0xcd + EX-0xFF
		} else
		...
		{
			//System.out.println("Interrupt mode 0: irqVector=" + Integer.toHexString(irqVector));
			//check for nop
			if (irqVector != 0) {
				...
				case 0xcd0000: //call
					...
					dec2SP();
					ram.writeWord(reg_SP, reg_PC);
					reg_PC = irqVector & 0xFFFF;
					tStates += 17;
					break;
				case 0xc30000: //jump
					reg_PC = irqVector & 0xFFFF;
					tStates += 10; //JP XXXX cycles
					break;
				default: //rst (or other opcodes ?)
					//push PC
					dec2SP();
					ram.writeWord(reg_SP, reg_PC);
					reg_PC = irqVector & 0x0038;
					tStates += 11; //RST xx cycles
					break;
				};
				
			}
			tStates += 2; //'interrupt latency' cycles
		}
	...
	public void executeOneInstruction() throws ProcessorException
	{
		...
		if (irqState != CLEAR_LINE && IFF1  && !afterEI) {
			takeInterrupt();
		}
		afterEI = false;
	...
	/**
	 * Execute up to the specified number of cycles.
	 */
	public void executeCycles(long maxCycles)
	{
		reset = false;
		try {
			...
			while (tStates < maxCycles) {
				...
				instruction = ram.readByte(reg_PC);
				//System.out.println("exec: " + (execCount++) + " Pc=0x" + Integer.toHexString(reg_PC) + " opcode=0x" + Integer.toHexString(instruction));
				incPC();
				decodeOneByteInstruction(instruction);
				if (reset) {
					reset = false;
					processorReset();
					tStates = maxCycles; //exit when reset is detected
				}
			}
			tStates -= maxCycles;
		} catch (ProcessorException e) {
	...
	public void executeSetInput(int inputNum, int state) {
		switch (inputNum) {
		case INPUT_LINE_NMI:
			//mark an NMI pending on the rising edge
			if (nmiState == CLEAR_LINE && state != CLEAR_LINE)  {
				NMI_FF = true;		
			}
			nmiState = state;
			break;
	 ...
	 */
	private final void decodeOneByteInstruction(int opcode) throws ProcessorException
	{
	...
	private void extendedCB()
	{
		...
		//System.out.println("  extended CB inst=0x" + Integer.toHexString(instruction));
		// decode stage
		switch (instruction)
		{
			case 0x00:
			...
			switch (instruction)
			{
				...
				default:
				{
					...
					throw new ProcessorException(ProcessorException.COMPUTER_UNIMPLEMENTED_OPCODE +
							" extendedED() opcode: 0x" + Integer.toHexString(instruction) );
				}
			}
		}
	}
			...
			switch (instruction)
			{
				...
				*/ 
				...
				default :
				{
					decPC();
					throw new ProcessorException(ProcessorException.COMPUTER_UNIMPLEMENTED_OPCODE +
							" extendedDDFD() opcode: 0x" + Integer.toHexString(instruction));
				}
			}
		}
	}
	...
	private void extendedIndexCB() throws ProcessorException
	{
...

		{
			switch (instruction)
			{
				...
				{
					throw new ProcessorException(ProcessorException.COMPUTER_UNIMPLEMENTED_OPCODE
							+ " extendedIndexCB() opcode 0x" + Integer.toHexString(instruction));
				}
				//
			}
		}
